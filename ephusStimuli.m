classdef ephusStimuli
    ...
    methods(Static)
      function step = makeStep(base, length, trailing, amplitude,Fs)
       %time = linspace(0,(base + length + trailing), (base + length + trailing) *Fs);
        step(1,[1:base*Fs]) = 0;
        step(1,[(base*Fs + 1): (base + length)*Fs]) = amplitude;   
        step(1,[((base + length)*Fs+1):(base + length+trailing)*Fs]) = 0;
      end

      function filtered = lpFilter(signal)
        Fcut = 180;
          [num,den]= besself(5,Fcut);
        [numd,dend]=bilinear(num,den,40000);
        filtered = filter(numd,dend,signal);
      end

      function filtered = lpFilterFc(signal, Fcut)
        [num,den]= besself(5,Fcut);
        [numd,dend]=bilinear(num,den,40000);
    ...
    function steps = makeStepSeries(amplitudes, length, isi,Fs) 
     ...
     steps = ephusStimuli.lpFilter(steps);
    end
    ...
    function [fWNstep,lWNstep] = filteredWNStep(base, length, trailing, amplitude, Fs)
        ...
        noisePower = 1;
        step = zeros(1,(base + length + trailing)*Fs);
        aStep = (awgn(step(1,[(base*Fs + 1): (base + length)*Fs]),noisePower) + 1) .* amplitude;
        step(1,[(base*Fs + 1): (base + length)*Fs]) = aStep;
        lWNstep = step;
