...
call %DEV_TOOL_HOME%\BuildInfo.bat
set "NIMEX_COMPILE_NON_MEX=%VS_COMPILER% /Zp4 /O2 /nologo /I. /c -DNIMEX_LOCALTIME_DEPRECATED -DNIMEX_COMPILER_CL %WIN_INCLUDES% %NIDAQMX_INCLUDES% %GLIB_INCLUDES% %MATLAB_INCLUDES%"
set "NIMEX_COMPILE_MEX=%NIMEX_COMPILE_NON_MEX% -DMATLAB_MEX_FILE"
set "NIMEX_LINK_NON_MEX=%VS_LINKER% /nologo /SUBSYSTEM:console %WIN_LIBS% %NIDAQMX_LIBS% %GLIB_LIBS% %MATLAB_LIBS% /DLL"
set "NIMEX_LINK_MEX=%NIMEX_LINK_NON_MEX% NIMEX_shared.lib /EXPORT:mexFunction"
set TARGET_PATH=.\win32

echo Generating build-time header file...
%NIMEX_COMPILE_NON_MEX% /FoNIMEX_BuildInfo.obj ..\src\c\NIMEX_BuildInfo.c
%VS_LINKER% /nologo /SUBSYSTEM:console %WIN_LIBS% %NIDAQMX_LIBS% %GLIB_LIBS% %MATLAB_LIBS% NIMEX_BuildInfo.obj /OUT:NIMEX_BuildInfo.exe
