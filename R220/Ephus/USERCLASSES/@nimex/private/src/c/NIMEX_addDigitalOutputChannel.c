/**
 * @file NIMEX_addDigitalOutputChannel.c
 *
 * @brief Creates a new digital input channel definition attached to the specified task.
 *
 * <b>Syntax:</b>\n
 * <tt>NIMEX_addDigitalOutputChannel(taskDefinition, physicalName)</tt>
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nlhs != 0)
        mexErrMsgTxt("NIMEX_addDigitalOutputChannel: No return arguments supported.");
    
    //There must be a NIMEX_TaskDefinition, a channel name, and then an even number of key-value pairs.
    if (nrhs < 2)
        mexErrMsgTxt("NIMEX_addDigitalOutputChannel: Insufficient arguments (a task definition and channel specification must be provided).");
    
    //taskDefinition = (NIMEX_TaskDefinition *)mxGetPr(prhs[0]);
    taskDefinition = (NIMEX_TaskDefinition *)NIMEX_unpackPointer(prhs[0]);
    if (taskDefinition == NULL)
        mexErrMsgTxt("NIMEX_addDigitalOutputChannel: Invalid (uninitialized) NIMEX_TaskDefinition pointer value: NULL");

    NIMEX_acquireLock(taskDefinition, NIMEX_DEFAULT_THREAD_TIMEOUT);//TO012407B
    
    //TO012907D
    channelDefinition = NIMEX_createChannelDefinition(NIMEX_DIGITAL_OUTPUT, mxArrayToString(prhs[1]));

    //Add the channel to the task definition.
    NIMEX_addChannelToList(taskDefinition, channelDefinition);

    NIMEX_MACRO_VERBOSE("NIMEX_addDigitalOutputChannel: \n")
    #ifdef NIMEX_VERBOSE
