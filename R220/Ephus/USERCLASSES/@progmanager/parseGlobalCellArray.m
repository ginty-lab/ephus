...
%      TO081810A: Fixed VI072808A which didn't actually check what the datatype of the callback was, it just looked at the current Matlab version. -- Tim O'Connor 8/18/10
%      BSTO100311A: Cleaned up the handling of variables that are specified more than once per program. -- Tim O'Connor 10/3/11
%      TO100411A: Since we now check for duplicate variables, only insert hObject if it does not already exist. -- Tim O'Connor 10/4/11
...
% get(gui_fig_handle)
    ...
    %globalarray = cat(2, globalarray, {'hObject', gui_fig_handle, 'Class', 'figure_handle', 'Config', 0});%TO122204a, TO120105C %TO100411A
catch
...
while counter <= length(globalarray)
    ...
    elseif any(strcmpi(param, {'class', 'min', 'max'}))
        GUI_UserData.(param) = value;
        value_assigned = 0;
    elseif  strcmpi(param, 'config')
        ...
        if ~value_assigned && ~isempty(GUI_UserData)
            ...
            if isfield(GUI_UserData, 'Gui')  % There is a GUI tied to this variable...
                ...
                for guicounter = 1 : length(progmanagerglobal.programs.(program_name).(gui_name).variableGUIs.(currentvariablename))
                    ...
                    %TO081810A
                    %VI072808A - Handle 2008b compatibility issues
        ...
        %BSTO100311A - It should be considered something of an "event" when a variable gets specified more than once. And corrective steps may be necessary. -- Tim O'Connor 10/3/11
        if ~isfield(progmanagerglobal.programs.(program_name).(gui_name).variables,param)
            ...
            fprintf(2, 'Warning: Duplicate instance found for ''%s'' in %s:%s.\n', param, program_name, gui_name);
        end
...
%TO100411A
if ~any(strcmpi(fieldnames(progmanagerglobal.programs.(program_name).(gui_name)), 'hobject'))
    progmanagerglobal.programs.(program_name).(gui_name).variables.hObject = gui_fig_handle;
    progmanagerglobal.programs.(program_name).(gui_name).configflags.hObject = 0;
end
