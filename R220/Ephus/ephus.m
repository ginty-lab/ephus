...
%% TIMING/CLOCK CONFIG
    ...
    digStimNultiBoardErrMsg = 'Sample clock origin/destination must both be configured when using digital stimulator channels and multiple boards.';
    if ~isempty(sampleClockOrigin)
        ...
        if ~isempty(sampleClockDestination)
            sampClockInternalOrigin = [sampleClockOrigin 'InternalOutput'];
            sampClockInternalDestination = regexprep(sampleClockOrigin,'(?<devName>\S*?)/ctr\d*',['$<devName>/' sampleClockDestination]);
            nimex_connectTerms(nimexEngine,sampClockInternalOrigin,sampClockInternalDestination);
        end        
    elseif ~isempty(digStimChannelNames)
        if length(uniqueBoardIDs) > 1
            error(digStimNultiBoardErrMsg);
...
%% START PROGRAMS
   ...
   if activeAmps
    ...
    % BS100311b allow use of photodiode without mapper
    if pockelsOn
    ...
    elseif ~isempty(photodiodeBoardID) && ~isempty(photodiodeChannelID) % 2011-04-15 Ben Suter -- photodiode may be configured without Pockels
        if waitbarUpdate(.72, wb, 'Starting photodiode configuration...');return;end
        pdiode = openprogram(progmanager, {'photodiode', 'photodiode', 'photodiodeConfiguration'},'photodiode1');
    else
        pdiode = [];
    ...
    if mapperEnabled
        ...
        setLocalBatch(progmanager, mapper, 'xVideoScaleFactor', xVideoImageSize, 'yVideoScaleFactor', yVideoImageSize, 'sampleRate', initialSampleRate);%<<<<<<<<<<---------- CONFIG   % BS100311a also set the sampleRate
        if pockelsOn
            setLocalBatch(progmanager, mapper, 'modulatorMin', pockelsModulatorMin); % 2011-05-02 Ben Suter -- added param to startup file for min value
            setLocalBatch(progmanager, mapper, 'modulatorMax', pockelsModulatorMax);
            
            mapper_enablePockelsCell;
            
% 2011-05-02 Ben Suter -- moved themin/max limit set to just before the mapper_enablePockelsCell
% call, so that the calibration at startup uses these configured values, instead of the hard-coded (0 / 2 V) values
% from (I think) mapper.makeGlobalCellArray()
%             setLocalBatch(progmanager, mapper, 'modulatorMax', pockelsModulatorMax); 
...
%Warn if there's more than one board and the sampleClockOrigin/Destination have not been set
if ~isempty(uniqueBoardIDs) && ~isscalar(uniqueBoardIDs) && (isempty(sampleClockOrigin) || isempty(sampleClockDestination))
   h = warndlg('When using multiple boards, it is strongly recommended to configure the sample clock origin and destination in the Ephus initialization file. Otherwise, timing drift will appear over time between channels on different boards.','Multiple Boards Without Master Clock','modal');
   uiwait(h);
end
