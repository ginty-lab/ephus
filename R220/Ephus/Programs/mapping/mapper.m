...
%
% BS061111A: split some lines for easier debugging, and print transformed
% minimal tranmission command to the command line.
% BS061211: Use the new return argument from Pockels calibration, the
% lookup table, and store this as a property for later use.
% BS100311: makeGlobalCellArray set sampleRate twice, removed second line
%
...
% 2011-06-20, Ben Suter: enable these two lines to allow for 40000 or 40k
% or 40 kHz sampling rate while flashing laser, i.e. to measure
% laser-evoked action-potentials.
% sampleRate = 40000; % Ben 2011-06-20
% setLocalBatch(progmanager, hObject, 'sampleRate', sampleRate);
...
%             getLocal(progmanager, hObject, 'modulatorMin'), getLocal(progmanager, hObject, 'modulatorMax')); 
        [coeffs, maxV, minV, offset, pockels_lookup] = getPockelsCalibrationFit(getDeviceNameByChannelName(job, 'pockelsCell'), getDeviceNameByChannelName(job, 'photodiode1'), ...
            getTriggerOrigin(daqjob('acquisition')), pockelsTrigger, photodiodeTrigger, getDeviceNameByChannelName(job, 'shutter0'), ...
            getLocal(progmanager, hObject, 'modulatorMin'), getLocal(progmanager, hObject, 'modulatorMax')); 
    else
        %TO080108E
...
setLocal(progmanager, hObject, 'pockels_lookup', pockels_lookup);
...
%BS061011A - split into two lines, for ease of debugging
minPowerCommand = mapper_pockelsCellPreprocessor(hObject, 0);
putSample(daqjob('acquisition'), 'pockelsCell', minPowerCommand);
disp(sprintf('Pockels minPowerCommand (0 percent): %g', minPowerCommand));
end
