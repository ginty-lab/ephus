...
%  TO100411B - Stop the pulseJacker (and mapper?) too. -- Tim O'Connor 10/4/11
%
...
%TO100411B - Stop the pulseJacker (and mapper?) too.
try
    if isprogram(progmanager, 'pulseJacker')
        pjObj = getGlobal(progmanager, 'hObject', 'pulseJacker', 'pulseJacker');
        if getLocal(progmanager, pjObj, 'enable')
            setLocal(progmanager, pjObj, 'enable', 0);
            pulseJacker('enable_Callback', pjObj, [], guidata(pjObj));
        end
    end
catch
    fprintf(1, 'Failed to stop pulseJacker when starting scope: %s\n', getLastErrorStack);
end
try
    if isprogram(progmanager, 'mapper')
        mapperObj = getGlobal(progmanager, 'hObject', 'mapper', 'mapper');
        if getLocal(progmanager, mapperObj, 'map')
            setLocal(progmanager, mapperObj, 'map', 0);
            pulseJacker('map_Callback', mapperObj, [], guidata(mapperObj));
        end
    end
catch
    fprintf(1, 'Failed to stop mapper when starting scope: %s\n', getLastErrorStack);
end
