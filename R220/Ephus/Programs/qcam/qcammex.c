 ...
 *  BS073113A - Support for "strobeHigh" parameter value to "setTriggerType" command
 *  TO080813G - Add in strobeLow too. -- Tim O'Connor 8/8/13
 *
 * Created
...
#define __cplusplus//Prevent the double definition of the bool enum within the QImgTypes.h header
#include "QImgTypes.h"
#undef __cplusplus
//QImaging - Camera Control
#include "QCamApi.h"
...
void qmx_setTriggerToStrobeHi(void) //BS073113A
{
    qmx_triggerType = qcTriggerStrobeHi;
    QCam_SetParam(&qmx_camSettings, qprmTriggerType, qcTriggerStrobeHi);
    return;
}

void qmx_setTriggerToStrobeLow(void) //BS073113A
{
    qmx_triggerType = qcTriggerStrobeLow;
    QCam_SetParam(&qmx_camSettings, qprmTriggerType, qcTriggerStrobeLow);
    return;
}

void qmx_setTriggerToPulseHigh(void) //BS073113A
{
    qmx_triggerType = qcTriggerPulseHi;
    QCam_SetParam(&qmx_camSettings, qprmTriggerType, qcTriggerPulseHi);
    return;
}

void qmx_setTriggerToPulseLow(void) //BS073113A
{
    qmx_triggerType = qcTriggerPulseLow;
    QCam_SetParam(&qmx_camSettings, qprmTriggerType, qcTriggerPulseLow);
    return;
}
...
//            free(qmx_baseFilename);
    ...
    else if (stringcompare(command, "setTriggerType") == 0)
    {
		...
		else if (stringcompare(mxArrayToString(prhs[1]), "strobeHigh") == 0)
            qmx_setTriggerToStrobeHi();
		else if (stringcompare(mxArrayToString(prhs[1]), "strobeLow") == 0)
            qmx_setTriggerToStrobeLow();
		else if (stringcompare(mxArrayToString(prhs[1]), "pulseHigh") == 0)
            qmx_setTriggerToPulseHigh();
		else if (stringcompare(mxArrayToString(prhs[1]), "pulseLow") == 0)
            qmx_setTriggerToPulseLow();
        else
